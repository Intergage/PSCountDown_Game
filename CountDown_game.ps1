...
# I'm not doing this manually ... Maybe I should have.
$cons = @()
$vowel = @()
...
function CheckSpelling () {
    Param([string]$str)
    $data = Invoke-RestMethod "https://api.cognitive.microsoft.com/bing/v5.0/spellcheck?text=$str&count=3&mkt=en-us" -Headers @{ "Ocp-Apim-Subscription-Key" = $bingAPI }
...
function CheckWord () {
    ...
    try {
        $data = Invoke-RestMethod "https://od-api.oxforddictionaries.com/api/v1/entries/en/$str" -Headers @{"app_id" = $oxappID; "app_key" = $oxAPI}
    }
    catch {
